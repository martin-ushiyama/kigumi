...
**[ブラウザで試す](https://martin-ushiyama.github.io/kigumi/)** · English: **[README.md](README.md)**
...
> 現在の編集環境はPC向けです。キーボードとマウスまたはトラックパッドが必要です。
...
- 編集内容はブラウザ内へ自動保存され、次回起動時に復元される
- 自動保存はこのブラウザのサイトデータにだけ残る。サイトデータの消去、プライベートブラウズ、端末の故障などで失われるため、「保存」から JSON を定期的にダウンロードしてバックアップする
- 「読込」で保存した JSON を復元できる (レシピも同梱)。別の端末や人との共有も JSON ファイルを受け渡して行う
- サーバーやクラウドへの保存・共有機能はない
